...
package eturlia.core.mixin.server;
 ...
 * <h2>Configuration</h2>
 * <ul>
 *   <li>{@code eturlia.thread.validation.enabled} (default: {@code true}) —
 *       Set to {@code false} to completely disable thread validation.</li>
 *   <li>{@code eturlia.thread.validation.strict} (default: {@code false}) —
...
public final class RegionThreadValidatorHooks {
    ...
    private static final Logger LOGGER = Logger.getLogger("EturliaRegionValidator");

    /** System property to enable/disable thread validation. */
    private static final String PROP_ENABLED = "eturlia.thread.validation.enabled";

    /** System property for strict (throw) vs. permissive (warn) mode. */
    private static final String PROP_STRICT = "eturlia.thread.validation.strict";
     ...
     * Eturlia's validation does not break vanilla/Spigot/Paper servers that
    ...
    private static void handleViolation(Object level, Thread current) {
        ...
        String message = "[Eturlia] Region thread validation FAILED: ServerLevel.tick() "
    ...
    private static void ensurePaperApiResolved() {
        ...
        if (paperApiResolved.compareAndSet(false, true)) {
            try {
                ...
                LOGGER.info("[Eturlia] RegionizedWorldThread resolved: "
                        + rwtClass.getName()
