...
import com.eturlia.compat.sable.handlers.CrossRegionSubLevelManager;
import com.eturlia.compat.sable.physics.SablePhysicsRegionBridge;
import com.eturlia.compat.sable.physics.JNIThreadSafetyAuditor;
import com.eturlia.compat.sable.handlers.VehicleAssemblyRegionHandler;
...
/**
 * Main entry point for the Eturlia compatibility layer for Sable + Create Aeronautics.
 *
 ...
 */
@Mod(EturliaCompatSable.MOD_ID)
public class EturliaCompatSable {

    /** Mod identifier used in {@code neoforge.mods.toml} and registration. */
    public static final String MOD_ID = "eturlia_compat_sable";
     ...
     */
    public EturliaCompatSable(IEventBus modEventBus, ModContainer modContainer) {
        this.modContainer = modContainer;

        LOGGER.info("[EturliaCompatSable] Initializing Sable + Create Aeronautics compat for Folia regionized server");
    ...
    private void onCommonSetup(final FMLCommonSetupEvent event) {
        LOGGER.info("[EturliaCompatSable] Common setup: registering region-aware handlers");
        ...
        event.enqueueWork(() -> {
            ...
            LOGGER.info("[EturliaCompatSable] All Sable region-aware handlers registered successfully");
        });
    }
}
